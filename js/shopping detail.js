...
preview.addEventListener('mousemove', function (e) {
    ...
    var half = fangda.offsetWidth / 2;
    var maxX = preview.offsetWidth - fangda.offsetWidth;
    var maxY = preview.offsetHeight - fangda.offsetHeight;
    var fangdaX = x - half;
    var fangdaY = y - half;
    if (fangdaX < 0) fangdaX = 0;
    if (fangdaY < 0) fangdaY = 0;
    if (fangdaX > maxX) fangdaX = maxX;
    if (fangdaY > maxY) fangdaY = maxY;
    fangda.style.left = fangdaX + 'px';
    fangda.style.top = fangdaY + 'px';
    // 放大图移动
    var scale = realImg.offsetWidth / preview.offsetWidth;
    realImg.style.left = -fangdaX * scale + 'px';
    realImg.style.top = -fangdaY * scale + 'px';
});
...
for (let i = 0; i < btns.length; i++) {
    ...
    //         if (e.target.getAttribute('isSelected') === 'false') { btns[i].style.borderColor = 'black' }
    //     })
    // })
}
